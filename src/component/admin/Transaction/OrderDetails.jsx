const OrderDetails = ({orderData}) =>{
    ...
    return(
         <div className="bg-white rounded-none shadow-sm border border-gray-200 p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-4 uppercase tracking-wider">Order Items</h2>
            ...
            <div className="flex-1">
                <h3 className="text-sm font-medium text-gray-900">{orderData?.name}</h3>
                <p className="text-sm text-gray-600 font-mono">{orderData?.sku}</p>
                <p className="text-sm text-gray-600">{orderData?.category}</p>
            </div>
            <div className="text-right">
                <p className="text-sm text-gray-600">Qty: {orderData?.total_items}</p>
                <p className="text-sm font-medium text-gray-900">₹{orderData?.total_amount.toLocaleString('en-IN')}</p>
            </div>
            </div>
            </div>
