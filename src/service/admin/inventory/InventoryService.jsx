...
export const getProductStats = async(formData, toast) => {
    try {
        const response =  await request.read(endPoint.admin.productStats, toast, {service: SERVICE_CONFIGS.ADMIN_SERVICE, requiresAuth: true}, {headers: { 'X-Store-Origin': getStoreName()}});
        if (response.success === true) return response
    } catch (error) {
        toast.error(error.message);
    }
