...
const errorHandler = (error, toast = null, options = {}) => {
    ...
    if (!response) {
        ...
        const message =  response?.data?.detail || response?.data?.error || response?.data?.data?.non_field_errors?.[0] || response?.data?.non_field_errors?.[0]||  response?.data?.error || response.data && response.data.message ||response?.data?.[0]|| response?.data?.message || codeMessage[response.status];
